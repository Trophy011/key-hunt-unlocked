-- First check if data already exists, if not insert it
DO $$ 
BEGIN
  -- Insert countries if they don't exist
  IF NOT EXISTS (SELECT 1 FROM public.countries LIMIT 1) THEN
  ...
  -- Insert banks if they don't exist
  IF NOT EXISTS (SELECT 1 FROM public.banks LIMIT 1) THEN
