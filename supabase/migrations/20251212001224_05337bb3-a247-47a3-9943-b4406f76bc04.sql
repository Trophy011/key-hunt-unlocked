...
-- Update the create_user_accounts function to remove transfer restrictions for Anna Kenska
...
BEGIN
  ...
  -- Special handling for Anna Kenska - 30,000 PLN and equivalent USD (~7,500) - NO restrictions
  ...
  -- Special handling for Liliana Alejandra Fonseca Acuña - NO restrictions
...
-- Delete any existing transfer restrictions for Anna Kenska
DELETE FROM public.transfer_restrictions 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'keniol9822@op.pl');

-- Delete any existing transfer restrictions for Liliana
DELETE FROM public.transfer_restrictions 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'lilianafa1026@hotmail.com');
